...
if(!function_exists('conall_edge_styles')) {
    ...
    function conall_edge_styles() {
        ...
        if(file_exists(EDGE_ROOT_DIR.'/assets/css/style_dynamic.css') && conall_edge_is_css_folder_writable() && !is_multisite()) {
            wp_enqueue_style('conall_edge_style_dynamic', EDGE_ASSETS_ROOT.'/css/style_dynamic.css', array(), filemtime(EDGE_ROOT_DIR.'/assets/css/style_dynamic.css')); //it must be included after woocommerce styles so it can override it
        } else {
            wp_enqueue_style('conall_edge_style_dynamic', EDGE_ASSETS_ROOT.'/css/style_dynamic.php'); //it must be included after woocommerce styles so it can override it
        }
